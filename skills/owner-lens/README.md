...
정확도는 폐쇄 자료보다 떨어진다. 숨기지 않고 등급으로 표기한다.
...
- 그래서 흉내 내지 않는다. **추적 가능한 버전**을 만든다.
  출처 대장·검증 로그·재구성 실패 영역을 붙인다. 원본에는 없는 것들이다.
- 한 사람의 인사이트 원장은 몇 건짜리 경험칙이지만,
  **여럿이 모으면 폐쇄 벤치마크 DB의 대체물**이 된다. 그것이 이 프로젝트의 끝 그림이다.
...
이건 각주가 아니라 **통과/탈락 판정**이다. 전부 통과하면 낙관 편향으로 보고 다시 계산한다.
...
C는 벤더 주장을 검증하게 하고, D는 폐쇄 자료 없이 시작할 수 있게 한다.
...
references/
  ...
  synthesis.md               ★ 공개 뉴스로 폐쇄 자료 재구성 (검증 6단계)
  substitution.md            폐쇄 자료 → 공개 대체물 대응표
