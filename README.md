...
인터뷰·공시·업계 동향으로 재구성해서 내 시각과 맞붙인다.
...
| **잘 만들었는데 안 쓰인다** | 사람이 손으로 정보를 옮기는 지점(단절)마다 묻는다 — *"여기 데이터를 안 넣으면 누가 곤란해지는가?"* 아무도 안 곤란하면 넣어도 안 쓰인다 |
...
한 사람의 원장(현장에서 얻은 배움을 모아 두는 장부)은 몇 건짜리 경험칙이지만,
여럿이 모이면 독점 벤치마크를 대신할 자료가 된다.
...
An example. A car-parts maker is considering **automated production
reporting**. Today a floor supervisor hand-tallies the numbers in Excel and
reports them the next morning. The vendor says the data is structured, so
automation is easy. The floor sees it differently:
