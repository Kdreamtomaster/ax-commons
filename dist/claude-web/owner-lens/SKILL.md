---
name: owner-lens
description: 공개 신호만으로 고객사 오너의 비즈니스 모델과 목적함수를 역설계하고, 파는 쪽·사는 쪽·일하는 쪽 중 어디에 서 있든 나머지 두 자리의 시각을 인터뷰·공시·업계 동향으로 재구성해 맞붙인다. 문답으로 정보를 받아가며 단계적으로 구체화한다. 초도 미팅 준비, 고객사 사전조사, 제안 전 분석, "이 회사가 뭘 원하는지 모르겠다", "오너 관점을 알아야 한다", 채용공고·IR·뉴스만으로 회사 파악하기, ROI가 임원 임기를 넘는지 검토, 미팅 후 가설 검증에 사용. 미팅 중 대화 자체를 시뮬레이션하려면 미팅 시뮬레이터 계열 스킬을 쓸 것.
---
