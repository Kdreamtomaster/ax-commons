...
| | 항목 | 이슈 | 상태 |
|---|---|---|---|
| Q1 | 시나리오 게이트 `0.6` ★ | [#1](https://github.com/Kdreamtomaster/ax-commons/issues/1) | 열림 |
...
| Q12 | ~~게이트를 레벨별로 낼 것인가~~ | [#12](https://github.com/Kdreamtomaster/ax-commons/issues/12) | **닫힘 — 필요 지평으로 해소** |
...
## Q1. 시나리오 게이트 `safe_ratio: 0.6` 이 맞는 값인가 ★

**지금 상태** 경험칙. 실측 없음. 이 프로젝트에서 제일 중요한 미검증 항목.

`owner-lens` 의 S7은 `필요 지평 = 회수 ÷ 0.6` 으로 계산한다 (구 임기 게이트, D-013).
0.6이라는 숫자에 근거가 없다. 게이트를 재설계해도 이 상수는 그대로 남았다.

**무엇이 관찰되면 결론이 나는가**

- 서로 다른 조직 10건 이상에서 게이트 판정과 실제 결과를 대조
- **인물 의존·조건부로 판정했는데 안건이 잘만 진행된 사례**가 나오면 게이트가 과하게 보수적이다
- 전 시나리오 성립으로 판정했는데 착수가 안 된 사례가 반복되면 **시나리오 재료에 빠진 변수**가 있다는 뜻

**이미 나온 의심** 예산 편성 주기. 논거가 성립해도 시즌이 지나면 내년으로 밀린다.
시나리오 재료에 예산 주기를 넣어야 할 수 있다.
...
R1 초안 우선 · `[불가]` 판정 · "남은 모름" · 반박 안내 · 시나리오 게이트 · 표 형식
...
**무엇이 관찰되면 결론이 나는가**
Codex에게 "시나리오 게이트를 돌려 달라"고 했을 때 규칙을 알고 있으면 읽은 것이다.
...
## Q12. ~~임기 게이트를 결정 레벨별로 내야 하는가~~ — 해소됨 (D-013)

**닫힌 이유** 시나리오 게이트의 **필요 지평**이 이 문제를 구조적으로 풀었다.

원래 문제: 만날 사람 한 명 기준으로 통과/탈락만 내니
"본부장 선에서는 막히고 대표 선에서는 살아난다"가 표현되지 않았다.

지금: 필요 지평(회수 ÷ 0.6)은 안건의 성질이라 사람과 무관하게 계산되고,
그 지평을 확실히 가진 결재선이 어디인지가 곧 **"이 결재선부터 성립한다"**다.
레벨별 잔여 시야를 추정할 필요가 없어졌다 — 사람을 논거로 쓰지 않기 때문이다.

→ [이슈 #12](https://github.com/Kdreamtomaster/ax-commons/issues/12) 닫힘 ·
경위는 [DECISIONS.md](../DECISIONS.md) D-013
